...
Check condition: All models/packages with stereotype `<<SecureAIService>>` need to have `{PackageIntegrityVerified}` set to  `true`.
...
Check condition: All models/packages with stereotype `<<SecureAIService>>` need to have `{RegularPackageUpdates}` set to  `true`.
...
Check condition: All models/packages with stereotype `<<SecureAIService>>` need to have `{PackagesFromSecureSources}` set to  `true`.
...
Check condition: All models/packages with stereotype `<<SecureAIService>>` need to have `{SecureDeployment}` set to  `true`.