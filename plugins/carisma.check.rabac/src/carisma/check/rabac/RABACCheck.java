...
import carisma.profile.umlsec.rabac.UMLsecRABAC;
...
public class RABACCheck implements CarismaCheckWithID {
	...
	public boolean perform(final Map<String, CheckParameter> parameters, final AnalysisHost host) {
		...
		if (model.getContents().get(0) instanceof Package) {
			...
			final var abac = UMLsecRABACUtil.getStereotypedElements(content, UMLsecRABAC.ABAC);
			...
			final var usersTag = UMLsecRABACUtil.getStringValues("roles", UMLsecRABAC.ABAC, abacClass);
			if (usersTag.isEmpty()) {
			...
			final var rightsTag = UMLsecRABACUtil.getStringValues("rights", UMLsecRABAC.ABAC, abacClass);
			...
			if (!this.sessions.isEmpty()) {
				...
				final var dsdTag = UMLsecRABACUtil.getStringValues("dsd", UMLsecRABAC.ABAC, abacClass);
				if (dsdTag.size() > 0) {
				...
				final var abacRequire = UMLsecRABACUtil.getStereotypedElements(content, UMLsecRABAC.ABACREQUIRE);
				...
				final var attributeFiltersTag = UMLsecRABACUtil.getStringValues("attributeFilters", UMLsecRABAC.ABAC,
						abacClass);
				...
				for (final String u : this.sessions.keySet()) {
					...
					for (final Element e : abacRequire) {
						final var right = UMLsecRABACUtil.getStringValues("right", UMLsecRABAC.ABACREQUIRE, e);
						if (right.isEmpty()) {
						...
						final var rhTag = UMLsecRABACUtil.getStringValues("rh", UMLsecRABAC.ABAC, abacClass);
						if (rhTag.size() > 0) {
						...
						for (final String r : roles) {
							if (rightsTag.get(0).contains("(" + r + "," + right.get(0) + ")")) {
								...
								final var filter = UMLsecRABACUtil.getStringValues("filters", UMLsecRABAC.ABACREQUIRE, e);
					...
					for (final List<List<Element>> m : paths) {
						...
						for (final List<Element> p : m) {
							...
							for (final Element e : p) {
								...
								if ((e instanceof Transition)
										&& !UMLsecRABACUtil.getStringValues("right", UMLsecRABAC.ABACREQUIRE, e).isEmpty()) {
								...
								for (final Element e : p) {
									if (validTransitions.contains(e)) {
										host.appendToReport(" (via right "
												+ UMLsecRABACUtil.getStringValues("right", UMLsecRABAC.ABACREQUIRE, e).get(0)
												+ ")");
									}
