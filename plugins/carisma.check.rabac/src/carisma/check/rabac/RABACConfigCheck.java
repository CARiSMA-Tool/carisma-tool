...
public class RABACConfigCheck implements CarismaCheckWithID, ActionListener {
	...
	public boolean perform(final Map<String, CheckParameter> parameters, final AnalysisHost host) {
		...
		if (model.getContents().get(0) instanceof Package) {
			...
			this.usersTag = UMLsecRABACUtil.getStringValues("roles", UMLsecRABAC.ABAC, abacClass);
			...
			final var abacAttribute = UMLsecRABACUtil.getStereotypedElements(content, UMLsecRABAC.ABACATTRIBUTE);
			for (final Element e : abacAttribute) {
				final var nameTag = UMLsecRABACUtil.getStringValues("name", UMLsecRABAC.ABACATTRIBUTE, e);
