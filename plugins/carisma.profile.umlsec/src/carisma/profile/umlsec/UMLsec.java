...
public enum UMLsec implements UMLsecProfile {

	SECURE_DEPENDENCY("secure dependency"), DATA_SECURITY("data security"), SECURE_LINKS("secure links"),
	NO_UP_FLOW("no up-flow"), NO_DOWN_FLOW("no down-flow"), GUARDED_ACCESS("guarded access"), RBAC("rbac"),
	CRITICAL("critical"), SEND("send"), SECRECY("secrecy"), PRIVACY("privacy"), INTEGRITY("integrity"), HIGH("high"),
	ENCRYPTED("encrypted"), WIRE("wire"), INTERNET("Internet"), LAN("LAN"), POS_DEVICE("POS device"),
	ISSUER_NODE("issuer node"), SMART_CARD("smart card"), PROVABLE("provable"), FAIR_EXCHANGE("fair exchange"),
	SEPERATION_OF_DUTY("seperation of duty"), ALLOWED_USERS("allowed users"), SAP_TRANSACTION("SAP Transaction"),
	USED_BY("used-by"), PROTECTED_ACTION("protected action"), AUTHORIZED_STATUS("authorized-status"),
	LOCKED_STATUS("locked-status"), IDENTIFIABLE("identifiable"), GUARDED("guarded"), CALL("call"),
	REQUIRES("requires");
	...
	private static final String PROFILE_NAME = "UMLsec";
	private static final String PROFILE_VERSION = "1";
	private static final String PROFILE_URI = "platform:/plugin/carisma.profile.umlsec/profile/UMLsec.profile.uml";
	...
	public boolean isEqual(Stereotype stereotype) {
		return contains(stereotype) && stereotype.getName().equalsIgnoreCase(getReadableName());
	}
