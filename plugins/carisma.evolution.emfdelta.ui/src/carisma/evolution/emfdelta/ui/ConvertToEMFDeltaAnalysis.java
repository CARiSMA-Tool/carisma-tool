 ...
 *
 */
public class ConvertToEMFDeltaAnalysis implements IObjectActionDelegate {
	 ...
	 */
	public ConvertToEMFDeltaAnalysis() {
		super();
	}
	 ...
	 */
	private static final String DID_NOT_FOUND_CHECK = "- did not find check '";
	
	/**
	 * Check-ID of the check which has to move to the first position.
	 */
	private final String firstString = "carisma.check.emfdeltaparser";
...
//		KR: ab hier änderungen um aus non evo checks evo checks zu machen
		for (int i = 3; i < checkList.size() - 1; i++) { //ersten drei sind nicht zu behanlden, letzte auch nicht
			...
			if (evoID != null) {
				checkList.remove(i);
				CheckReference checkRef = Carisma.getInstance().getCheckRegistry().createReference(Carisma.getInstance().getCheckRegistry().getCheckDescriptor(evoID));
		...
		MessageDialog.openInformation(
				shell,
				"CARiSMA",
//				"! This is not the final functionality of this action !\n"
				""
				+ "Successfully converted " + selectedFile.getName() + " to an EMFDelta analysis\n" 
				+ "Actions performed:\n" 
