...
public class Check implements CarismaCheck {
	...
	public final boolean perform(final Map<String, CheckParameter> parameters, final AnalysisHost host) {
		...
		try {
			OutputFileParameter file = (OutputFileParameter) parameters.get("carisma.check.activity2petrinet.destFile");
		...
		if (currentModel.getContents().get(0) instanceof Model) {
			...
			if (parameters.containsKey("carisma.check.acivity2petrinet.page")) {
				if (parameters.get("carisma.check.acivity2petrinet.page") instanceof BooleanParameter) {
					withPage = ((BooleanParameter) parameters.get("carisma.check.acivity2petrinet.page")).getValue();
				}
			}
