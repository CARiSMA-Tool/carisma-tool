...
public class CreateHelpDocumentCheck implements CarismaCheckWithID {
	
	// Check ID
	public static final String CHECK_ID = "carisma.check.createHelpDocument"; //$NON-NLS-1$

	// Param ID
	public static final String PARAM_STS_FILE_OR_DB = "carisma.check.createHelpDocument.STSFileOrDB"; //$NON-NLS-1$
	
	public static final String CHECK_NAME = "Create Help Document for STS mapping"; //$NON-NLS-1$
	...
	public boolean perform(Map<String, CheckParameter> parameters, AnalysisHost host) {
		...
		if (model.getContents().get(0) instanceof Package) {
			...
			try {
				BooleanParameter fileOrDB = (BooleanParameter) (parameters.get(CreateHelpDocumentCheck.PARAM_STS_FILE_OR_DB));
				...
				for (int m = 0; m < UMLelements.size(); m++) {
					...
					if (n instanceof org.eclipse.uml2.uml.Class) {
