...
/**
 * Check-Class to manage the Class StateMachinePaths.
 * @author ---
 *
 */
public class Check implements CarismaCheck {
	...
	public final boolean perform(final Map<String, CheckParameter> parameters, final AnalysisHost host) {
		...
		if (currentModel.getContents().get(0) instanceof Package) {
			...
			mach = UMLHelper.getAllElementsOfType(model, StateMachine.class);
			host.appendLineToReport(mach.size() + " StateMachines found.");
			for (int i = 0; i < mach.size(); i++) {
				p.getPaths((StateMachine) mach.get(i), host, false);
			}
			return true;
		}
		return false;
