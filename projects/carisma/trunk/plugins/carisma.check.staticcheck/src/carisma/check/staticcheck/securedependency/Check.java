...
public class Check implements CarismaCheck {
	...
	public boolean perform(Map<String, CheckParameter> parameters, AnalysisHost newHost) {
		...
		if (currentModel.getContents().get(0) instanceof Package) {
			...
			int violations = sdc.checkSecureDependency(model, onlyCheckUsages);
	...
	private void extractParameters(final Map<String, CheckParameter> parameters) {
		onlyCheckUsages = ((BooleanParameter) parameters.get(PARAM_ONLYCHECKUSAGES)).getValue();		
	}
	
	private String computeMessage(SecureDependencyViolation v) {
