...
import carisma.regulatory.ontology.Process;
...
public class OWL2Process extends OWL2RuleElement implements Process {

    public OWL2Process(OWL2Ontology onto, OWLNamedIndividual individual) {
    ...
    public List<Role> getParticipants() {
        ...
        for (OWLNamedIndividual i : set) {
            if (parent.getGenericOntologyHelper().getTypeOfIndividual(i).equals("Role")) {
                Role s = (Role) OWL2Factory.createRuleElementFromIndividual(parent, i);
                list.add(s);
            }
        }
        return list;
