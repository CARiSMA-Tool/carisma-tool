 ...
 */
public class MariskSoD implements CarismaCheck {
	...
	public final boolean perform(final Map<String, CheckParameter> parameters, final AnalysisHost host) {
		...
		if (parameters.containsKey("carisma.check.bpmn2.marisk.sod")) {
			String sodParameter = ((StringParameter) parameters.get(
					"carisma.check.bpmn2.marisk.sod")).getValue().trim();
			...
			if (sodParameter.equals("")) {
				...
				sodNameList = extractSoDActivites(sodParameter);
				sodActivityList = mapSoDActivities(sodNameList);
				List<String> missedMappingNames = checkProperSoDMapping(sodActivityList, sodNameList);
				...
				boolean sodCheck = performSeparationOfDutyTest(sodActivityList);
				if (sodCheck) {
					host.addResultMessage(new AnalysisResultMessage(
	 ...
	 */
	private boolean parseSoDActivities(final String sodParameter) {
	 ...
	 */
	private List<SoDEntity<String>> extractSoDActivites(final String sodParameter) {
	 ...
	 */
	private List<String> checkProperSoDMapping(final List<SoDEntity<Activity>> sodActivities, final List<SoDEntity<String>> sodNames) {
	 ...
	 */
	private boolean performSeparationOfDutyTest(final List<SoDEntity<Activity>> sodList) {
		boolean passed = true;
