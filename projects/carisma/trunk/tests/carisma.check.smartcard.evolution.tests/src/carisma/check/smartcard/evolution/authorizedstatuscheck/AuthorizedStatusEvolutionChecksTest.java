...
public class AuthorizedStatusEvolutionChecksTest {
	...
	private class TestHost implements AnalysisHost{
		...
		public Object getFromRegister(String registerName) {
			if (registerName.equals(AuthorizedStatusEvolutionModifierCheck.MODIFIERS_REGISTRY_KEY)) {
				return modifierMap;
			}
	...
	public final void init(final String modelfilename) {
		...
		model = TestHelper.loadModel(filepath, modelfilename);
		modifierMap = new ModifierMap(model.eResource());
	...
	public final void testASDeleteGuard() {
		...
		try {
			...
			Transition el = (Transition) UMLHelper.getElementByName(model, "Uebergang");
			assertNotNull(el);
			...
			deltas.add(new Delta(new ArrayList<String>(), content));
			assertFalse(theCheck.perform(null, testHost));
			State authorizedState = UMLHelper.getElementOfNameAndType(model, "State1", State.class);
			...
			deltas.clear();
			deltas.add(new Delta(new ArrayList<String>(), content));
			assertTrue(theCheck.perform(null, testHost));
	...
	public final void testASAddTransition() {
		...
		try {
			AuthorizedStatusEvolutionModifierCheck theCheck = new AuthorizedStatusEvolutionModifierCheck();
			Region region1 = (Region) UMLHelper.getElementByName(model, "Region");
			assertNotNull(region1);
			Transition transition = (Transition) UMLHelper.getElementByName(model, "Transition1");
			...
			deltas.add(new Delta(new ArrayList<String>(), content));
			assertFalse(theCheck.perform(null, testHost));
			Region targetRegion = UMLHelper.getElementOfNameAndType(model, "Region", Region.class);
	...
	public final void testASSubstPermission() {
		...
		try {
			State authorizedState = UMLHelper.getElementOfNameAndType(model, "Region::State1", State.class);
			assertNotNull(authorizedState);
			...
			deltas.add(new Delta(new ArrayList<String>(), deltaContent));
			
			AuthorizedStatusEvolutionModifierCheck theCheck = new AuthorizedStatusEvolutionModifierCheck();
			assertFalse(theCheck.perform(null, testHost));
			Transition incomingTransition = UMLHelper.getElementOfNameAndType(model, "Uebergang", Transition.class);
	...
	public final void testASSubstGuard() {
		...
		try {
			Transition ueber = UMLHelper.getElementOfNameAndType(model, "Uebergang", Transition.class);
			assertNotNull(ueber);
